...
if (isset($_GET['booking_id']) && isset($_GET['new_status']) && is_numeric($_GET['booking_id'])) {
    ...
    if (in_array($newStatus, ['pending', 'confirmed', 'cancelled', 'completed'])) {
        ...
        if (mysqli_query($conn, $updateQuery)) {
            ...
            $errorMessage = "Error updating booking status: " . mysqli_error($conn);
        }
    }
}
...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    if (mysqli_query($conn, $deleteQuery)) {
        ...
        $errorMessage = "Error deleting booking: " . mysqli_error($conn);
    }
}
...
if (!empty($search)) {
    $whereConditions[] = "(u.first_name LIKE '%$search%' OR u.last_name LIKE '%$search%' OR u.email LIKE '%$search%' OR r.room_type LIKE '%$search%')";
}
